...
install_canvas() {
    ...
    log_msg "Verifying presence of general key example configuration files post-clone..."
    key_example_files=(
        "config/database.yml.example"
        "config/domain.yml.example"
        ".env.example"
        # config/environments/production.rb.example will be checked more defensively in configure_canvas
    )
...
configure_canvas() {
    ...
    log_msg "Updating security.yml with a new encryption key..."
    ...
    # --- REVISED LOGIC FOR production.rb ---
    ...
    # The 'prod' branch might directly contain production.rb without .example
    # However, if production.rb.example exists, it's the canonical source.
    # The previous log showed production.rb.example was MISSING from the clone.
    # The log also showed production.rb (no .example) *was* present.
    # This implies that the 'prod' branch might ship with production.rb directly ready or
    # that production.rb.example is truly missing from the specific git commit being checked out.
    ...
    if [ -f "$prod_rb_source_example" ]; then
        ...
        # If .example is missing, but production.rb (target) exists (e.g. from git clone itself)
        # we will assume this existing production.rb is the intended base.
        # We back it up and then "copy" it to itself to normalize the flow,
        # though it's more about acknowledging it's the base.
        ...
        source_to_copy="$prod_rb_target" # We'll use this as the "source" to copy to target after backup.
    else
        log_error "CRITICAL: Neither '$prod_rb_source_example' nor a pre-existing '$prod_rb_target' found in $(pwd)/config/environments/."
        log_error "This means no suitable production environment template is available after git clone."
        log_error "Please check the Canvas LMS repository structure for the '$CANVAS_BRANCH' branch or network issues during clone."
        ls -la /var/canvas/config/environments/
        exit 1
    fi

    # Backup $prod_rb_target if it exists, before copying from source_to_copy
    if [ -f "$prod_rb_target" ]; then
        log_warn "$prod_rb_target already exists. Backing it up before creating/updating."
    ...
    # Any subsequent sed commands would apply to this newly copied/confirmed file.
    # For now, Canvas usually doesn't require sed changes to production.rb itself, mostly to yml files.
    ...
    if [ ! -f "$env_target_file" ]; then 
        if [ -f "$env_example_file" ]; then
            ...
            log_warn "$env_example_file not found, creating empty $env_target_file."
        fi
    else
...
compile_assets() {
    ...
    if ! yarn install --frozen-lockfile --check-files; then
        log_warn "yarn install --frozen-lockfile failed. Attempting 'yarn install' to update lockfile..."
        if ! yarn install; then
            log_error "yarn install still failed. Check Node.js/Yarn setup and available memory."
            exit 1
        fi
    fi
    ...
    if ! RAILS_ENV=production bundle exec rake canvas:compile_assets; then
        log_error "Asset compilation (rake canvas:compile_assets) failed. Check logs."
        exit 1
    fi
