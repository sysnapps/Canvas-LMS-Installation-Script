...
install_canvas() {
    ...
    log_msg "Listing key configuration files in /var/canvas to verify clone status..."
    ...
    # Create .env.example if it doesn't exist (newer Canvas versions might not include it)
...
configure_canvas() {
    ...
    if [ -f "$prod_rb_source_example" ]; then
        ...
        # Create a minimal production.rb as a fallback
...
# Standard Rails production environment
...
EOF
    ...
    if [ -f "$prod_rb_target" ] && [ "$source_to_copy" != "$prod_rb_target" ]; then
    ...
    elif [ "$source_to_copy" == "$prod_rb_target" ]; then
        log_msg "Using existing $prod_rb_target directly."
    fi
...
compile_assets() {
    ...
    # Try different approaches to handle potential issues with yarn install
    if ! yarn install --frozen-lockfile --check-files; then
        ...
        if ! yarn install; then
            log_warn "Standard yarn install failed. Attempting with --network-timeout 600000..."
            if ! yarn install --network-timeout 600000; then
                log_error "All yarn install attempts failed. Check Node.js/Yarn setup and available memory."
                exit 1
            fi
        fi
    fi
